...
double lrh_duration_prob(lrh_duration* src, FP_TYPE duration) {
  return fastexp(lrh_duration_prob_lg(src, duration));
}
...
FP_TYPE* lrh_forward(lrh_model* model, lrh_seg* seg, FP_TYPE* output_lg, int nt, FP_TYPE** ab_) {
  lrh_model_precompute(model);
  #undef VITERBI
...
FP_TYPE* lrh_backward(lrh_model* model, lrh_seg* seg, FP_TYPE* output_lg, int nt, FP_TYPE** bb_) {
  lrh_model_precompute(model);
...
lrh_pslice* lrh_durocp(lrh_model* model, lrh_seg* seg, FP_TYPE* forward, FP_TYPE* backward,
  ...
  int nseg = seg -> nseg;
  FP_TYPE* p_ = output_lg;
# define ae(t, i) forward [(t) * nseg + (i)]
# define be(t, i) backward[(t) * nseg + (i)]
  ...
  for_tj_forward(0, 0, 1, 2)
    ...
    for(int d = mindur; d < maxdur; d ++) {
      int t1 = t + d - 1;
      pobserv += t1 < nt ? p(t1, j) : 0;
      FP_TYPE pnext = t1 < nt ? be(t1, j) : 0;
      FP_TYPE pdur = lrh_duration_prob_lg(srcdr, d);
      FP_TYPE pd = pobserv + pnext + pdur + pprev - total;
